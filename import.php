...
function importProducts($file, $pdo)
{
    ...
    try {
        ...
        $insertSql = "
            INSERT INTO product (name, unit, price, typeid, categoryid, cuisineid, status)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            foreach ($sheet->getRowIterator() as $row) {
                ...
                if ($existingProductId) {
                    $pdo->prepare("UPDATE product SET unit = ?, price = ?, typeid = ?, categoryid = ?, cuisineid = ?, status = ? WHERE name = ?")
                        ->execute([$data[2], $data[3], $data[4], $data[5], $data[6], $data[7], $data[1]]);
                }
            }
        }
