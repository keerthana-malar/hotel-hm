...
{
    header("Location: $url" . urlencode($message));
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($stmt_check->fetchColumn() == 0) {
            ...
            if (!$stmt->execute()) {
                redirectTo($u2, 'Something Wrong, please try again later');
            } else {
                $wasteID = $pdo->lastInsertId();
                ...
                for ($i = 0; $i < count($_POST['pro']); $i++) {
                    ...
                    if ($quantity >= $eqty) {
                        ...
                        $delQstmt = $pdo->query($delQ);
                        if ($delQstmt->execute()) {
                            redirectTo($u2, 'Not valid qty for ' . $proname);
                        }
                    }
                }
            }
            ...
            for ($i = 0; $i < count($_POST['pro']); $i++) {
                ...
                if ($wasteItemStmt->execute()) {
                    ...
                    $upStQ = "UPDATE `stockitem` SET qty = :avlQty WHERE product_id = :productID";
                    $upStQStmt = $pdo->prepare($upStQ);
                    $upStQStmt->bindParam(':avlQty', $AvlQty);
            ...
            $pdo->commit();
            redirectTo($u1, 'Waste Successfully Created');
        } else {
            redirectTo($u2, 'A record already exists for this branch on the specified date');
        }
    } catch (PDOException $e) {
        $pdo->rollBack();
        redirectTo($u2, 'An error occurred: ' . $e->getMessage());
    }
}
